...
/** Backs the Dead Stock (Top 20) widget's ListView — see
...
class InventoryDeadStockWidgetService : RemoteViewsService() {
    override fun onGetViewFactory(intent: Intent): RemoteViewsFactory = InventoryDeadStockRemoteViewsFactory(applicationContext)
}

class InventoryDeadStockRemoteViewsFactory(private val context: android.content.Context) : RemoteViewsService.RemoteViewsFactory {
    private var rows: List<InventoryRepository.StockValueRow> = emptyList()

    override fun onCreate() {}
    override fun onDataSetChanged() { rows = InventoryDeadStockCache.rows }
