  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .app-bar-text h1 { font-size: 1.1rem; font-weight: 800; color: #fff; letter-spacing: -.3px; }
  .app-bar-text p  { font-size: 0.75rem; color: #94a3b8; font-weight: 500; margin-top: 2px; }
  ...
  /* ── ROW VARIANTS ── */
  .row.computed { background: #f8fafc; }
  .row.computed input[readonly] { background: #f1f5f9 !important; font-weight: 700; color: var(--navy); }
  ...
  .row.highlight-blue input { background: #dbeafe !important; color: var(--blue); border-color: #93c5fd; }
  .row.highlight-green { background: var(--green-pale); }
  .row.highlight-green label { color: var(--green); font-weight: 700; }
  .row.highlight-green input { background: #dcfce7 !important; color: var(--green); border-color: #86efac; }
  .row.highlight-red { background: var(--red-pale); }
  .row.highlight-red label { color: var(--red); font-weight: 700; }
  .row.highlight-red input { background: #fee2e2 !important; color: var(--red); border-color: #fca5a5; }
  .row.override-on input { background: var(--amber-pale) !important; border-color: var(--amber) !important; color: #92400e !important; }
  /* final-editable rows */
  .row.final-editable input[type="number"] { background: #fef9c3 !important; border-color: var(--amber) !important; cursor: pointer; }
  .row.final-editable label { color: #78350f; }
  ...
  /* strips 4-col row */
  .strips-header {
    display: flex; padding: 7px 16px; background: #f1f5f9;
  ...
  /* three-cell credit row */
  .row.three-col { gap: 6px; }
  .row.three-col select { width: auto; flex: 1; text-align: left; font-family: 'Inter', sans-serif; font-weight: 500; }
  .row.three-col input { width: 100px; }

  /* delete row button */
  .del-row-btn {
    background: none; border: none; color: #fca5a5; font-size: 1rem;
  ...
  .named-account-head {
    ...
    font-size: 0.82rem; font-weight: 700; color: var(--ink, #1e293b);
  }
  ...
  .btn-navy:hover { background: #1e293b; box-shadow: var(--shadow-md); }
  ...
  .btn-nav   { background: #e2e8f0; color: var(--slate); padding: 7px 14px; border-radius: var(--radius-sm); font-size:0.8rem; }
  ...
  .form-field input:focus, .form-field select:focus {
    outline: none; border-color: var(--teal); box-shadow: 0 0 0 3px rgba(13,148,136,.15); background: #fff;
  }
  ...
  .dot-night   { background: #6366f1; }
  .dot-morning { background: var(--amber); }
  .dot-evening { background: #f97316; }
  ...
  .banner-row:last-child { border: none; padding-top: 14px; font-size: 1.2rem; font-weight: 800; color: #fff; }
  .banner-row .val { font-family: 'JetBrains Mono', monospace; font-weight: 600; }
  .banner-row .val.pos { color: #4ade80; }
  .banner-row .val.neg { color: #f87171; }
  ...
  /* ── SETTINGS LIST ── */
  .settings-item { display: flex; gap: 8px; align-items: center; padding: 8px 16px; border-bottom: 1px solid #f1f5f9; }
  .settings-item input { padding: 6px 10px; border: 1px solid var(--border); border-radius: var(--radius-sm); font-size: 0.85rem; font-family: 'Inter', sans-serif; }
  .settings-item input:first-child { flex: 2; }
  .settings-item input:nth-child(2) { width: 90px; text-align: right; font-family: 'JetBrains Mono', monospace; }

  /* ── MANIFEST LIST ── */
  .manifest-item {
    display: flex; align-items: center; justify-content: space-between;
    padding: 9px 16px; border-bottom: 1px solid #f1f5f9; font-size: 0.83rem;
  }
  ...
  .notice {
    ...
    background: var(--amber-pale); border: 1px solid #fcd34d; border-radius: var(--radius-md);
    padding: 10px 14px; font-size: 0.8rem; color: #92400e; font-weight: 500; margin: 0;
  }
  ...
  .numpad-label { font-size: 0.8rem; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .5px; }
  .numpad-display {
    font-size: 1.8rem; font-weight: 700; color: var(--navy);
    font-family: 'JetBrains Mono', monospace;
    padding: 10px 20px 14px;
    text-align: right; min-height: 52px;
    border-bottom: 2px solid var(--teal);
    letter-spacing: -1px;
  }
  .numpad-display.has-val { color: var(--navy); }
  .numpad-display.empty-val { color: var(--muted); }
  ...
  .np-btn {
    background: var(--white); border: none; padding: 18px 0;
  ...
  .np-btn:active { background: var(--teal-light); }
  .np-btn.np-zero { grid-column: span 2; }
  .np-btn.np-back { background: #f8fafc; color: var(--red); font-size: 1.3rem; }
  .np-btn.np-clear { background: var(--red-pale); color: var(--red); font-size: 0.85rem; font-weight: 700; }
  .np-btn.np-confirm {
    background: var(--teal); color: #fff; font-size: 0.85rem; font-weight: 700;
    grid-column: span 3;
    padding: 16px 0;
  }
  .np-btn.np-confirm:active { background: var(--teal-dark); }
  .numpad-close-btn {
    background: none; border: none; font-size: 1.1rem; cursor: pointer; color: var(--muted);
    padding: 4px 8px; border-radius: var(--radius-sm);
  }
  .numpad-close-btn:hover { color: var(--navy); background: var(--surface); }
  ...
  .edit-modal-pin:focus { outline: none; border-color: var(--teal); }
  .edit-mode-toggle { display: flex; gap: 0; border: 1.5px solid var(--border); border-radius: var(--radius); overflow: hidden; }
  .edit-mode-btn { flex: 1; padding: 10px; border: none; background: #f8fafc; font-size: 0.85rem; font-weight: 600; color: var(--muted); cursor: pointer; transition: background .15s, color .15s; }
  .edit-mode-btn.active-shift { background: var(--navy); color: #fff; }
  .edit-mode-btn.active-final { background: #7c3aed; color: #fff; }
  ...
  #print-sheet.show { display:block; position:fixed; left:0; top:0; z-index:-1; background:#fff; }
  #print-sheet .ps-page {
    width: 794px; height: 1123px;
    background: #f7f5f0; color: #1c2b2b;
    font-family: 'Inter', Arial, sans-serif;
    position: relative; overflow: hidden; page-break-after: always; box-sizing: border-box;
  }
  #print-sheet .ps-page:last-child { page-break-after: auto; }
  #print-sheet .ps-spine {
    position: absolute; left: 0; top: 0; bottom: 0; width: 20px; background: #0b1e2d;
  }
  #print-sheet .ps-spine-final { background: #8a5a1e; }
  #print-sheet .ps-spine-label {
  ...
  #print-sheet .ps-spine-ticks { position:absolute; left:0; top:0; bottom:0; width:20px; }
  #print-sheet .ps-tick { position:absolute; left:4px; width:12px; height:1px; background:rgba(255,255,255,.12); }
  #print-sheet .ps-content { margin-left: 20px; padding: 10px 14px 6px; }
  #print-sheet .ps-letterhead {
    display:flex; align-items:center; justify-content:space-between;
    border-bottom: 1.5px solid #0b1e2d; padding-bottom: 6px; margin-bottom: 7px;
  }
  #print-sheet .ps-brand h1 {
    font-family: 'Source Serif 4', Georgia, serif;
    font-size: 15px; font-weight: 600; color: #0b1e2d;
  }
  #print-sheet .ps-brand p {
    font-size: 7px; color: #5b6b6b; letter-spacing: 1.4px; text-transform: uppercase; margin-top: 1px;
  }
  #print-sheet .ps-doctype { text-align: right; }
  #print-sheet .ps-doctype .ps-doctype-tag {
    display:inline-block; font-family:'JetBrains Mono',monospace; font-size:7.5px;
    letter-spacing: 2px; text-transform:uppercase; color:#fff;
    background:#0d7d72; padding: 2px 7px; border-radius: 3px;
  }
  #print-sheet .ps-page-final .ps-doctype .ps-doctype-tag { background:#8a5a1e; }
  #print-sheet .ps-doctype .ps-doctype-date { margin-top: 3px; font-size: 8.5px; color: #1c2b2b; font-weight: 600; }
  #print-sheet .ps-box {
    background:#fff; border:1px solid #e2ddd2; border-radius: 4px;
    overflow:hidden; margin-bottom: 5px;
  }
  #print-sheet .ps-box h4 {
    margin:0; padding: 3px 7px; font-size: 7px; font-weight:700;
    text-transform: uppercase; letter-spacing: 1px;
    background: #0b1e2d; color: #f7f5f0;
  }
  #print-sheet .ps-box.ps-box-accent h4 { background:#0d7d72; }
  #print-sheet .ps-box.ps-box-final h4  { background:#8a5a1e; }
  #print-sheet .ps-row {
    display:flex; justify-content:space-between; gap: 6px;
    padding: 2px 7px; font-size: 8px;
    border-bottom: 1px solid #f1efe9;
  }
  #print-sheet .ps-row:last-child { border-bottom:none; }
  #print-sheet .ps-row span:first-child { color:#4a5757; flex:1; }
  #print-sheet .ps-row span:last-child {
    font-family:'JetBrains Mono',monospace; font-weight:700; color:#0b1e2d; white-space:nowrap;
  }
  #print-sheet .ps-row.ps-total { background:#f1efe9; }
  #print-sheet .ps-row.ps-total span { color:#0b1e2d !important; font-weight:700; }
  #print-sheet .ps-row.ps-highlight { background:#e6f4f1; border-top:1px solid #0d7d72; border-bottom:1px solid #0d7d72; }
  #print-sheet .ps-row.ps-highlight span { color:#0d7d72 !important; font-weight:800; }
  #print-sheet .ps-row.ps-red { background:#fbeeec; }
  #print-sheet .ps-row.ps-red span { color:#c8463c !important; font-weight:700; }
  #print-sheet .ps-row.ps-minus span:last-child::before { content:'\2212\00A0'; }
  #print-sheet .ps-row.ps-plus  span:last-child::before { content:'+\00A0'; }
  #print-sheet .ps-row.ps-empty span:last-child { color:#c4bfae; font-weight:500; }
  #print-sheet .ps-main3 {
    display: grid; grid-template-columns: 1fr 1fr 1fr;
    gap: 5px; align-items: start; margin-bottom: 5px;
  }
  #print-sheet .ps-col { display: flex; flex-direction: column; gap: 5px; }
  #print-sheet .ps-grid2 { display:grid; grid-template-columns:1fr 1fr; gap:6px; align-items:start; }
  #print-sheet .ps-hero {
    border-radius: 4px; padding: 8px 12px; margin-top: 4px;
    display:flex; align-items:center; justify-content:space-between;
    background: linear-gradient(135deg,#0b1e2d 0%,#16323f 100%);
  }
  #print-sheet .ps-hero.ps-hero-short { background: linear-gradient(135deg,#5c1f1a 0%,#7a2c24 100%); }
  #print-sheet .ps-hero .ps-hero-label { font-size:7.5px; letter-spacing:1.4px; text-transform:uppercase; color:rgba(255,255,255,.6); }
  #print-sheet .ps-hero .ps-hero-val { font-family:'JetBrains Mono',monospace; font-size:18px; font-weight:800; color:#fff; margin-top:1px; }
  #print-sheet .ps-hero .ps-hero-sub { text-align:right; font-size:7.5px; color:rgba(255,255,255,.5); max-width:180px; line-height:1.4; }
  #print-sheet .ps-chiprow { display:flex; gap:4px; flex-wrap:wrap; margin-bottom:6px; }
  #print-sheet .ps-chip {
    background:#fff; border:1px solid #e2ddd2; border-radius:20px;
    padding:2px 7px; font-size:8px; font-family:'JetBrains Mono',monospace; color:#0b1e2d;
  }
  #print-sheet .ps-foot {
    position:absolute; bottom:6px; left:20px; right:0;
    padding:4px 14px 0; display:flex; justify-content:space-between;
    font-size:7px; color:#9a9484; border-top:1px solid #e2ddd2;
  }
  #print-sheet .ps-foot-page { font-family:'JetBrains Mono',monospace; }
  @media print {
    #print-sheet { display:block !important; position:static !important; z-index:auto !important; }
  ...
  .ltb-back:hover { background: #e2e8f0; }
  .ltb-title {
  ...
  .sync-card-head h3 { font-size: 0.85rem; font-weight: 700; color: #fff; letter-spacing: .3px; }
  .sync-card-head p  { font-size: 0.7rem; color: #94a3b8; margin-top: 1px; }
  .sync-card-body { padding: 16px 18px; }

  /* State A — unlinked */
  .sync-unlinked-text {
    font-size: 0.8rem; color: #94a3b8; line-height: 1.6; margin-bottom: 14px;
  }
  .sync-unlinked-text strong { color: #e2e8f0; }
  .btn-dropbox {
    display: inline-flex; align-items: center; gap: 8px;
    background: #0061ff; color: #fff; border: none; border-radius: var(--radius-md);
  ...
  .btn-dropbox:hover { background: #0052d9; box-shadow: 0 4px 14px rgba(0,97,255,.4); }
  .btn-dropbox:active { transform: translateY(1px); }
  ...
  .sync-connected-badge .sync-dot {
    ...
    box-shadow: 0 0 6px #4ade80; animation: syncPulse 2s infinite;
  }
  @keyframes syncPulse {
    0%, 100% { opacity: 1; } 50% { opacity: .4; }
  }
  .sync-status-line {
    font-size: 0.75rem; color: #94a3b8; font-family: 'JetBrains Mono', monospace;
  ...
  .sync-status-line.status-ok     { color: #4ade80; border-color: rgba(74,222,128,.2); }
  .sync-status-line.status-busy   { color: #fbbf24; border-color: rgba(251,191,36,.2); }
  .sync-status-line.status-error  { color: #f87171; border-color: rgba(248,113,113,.2); }
  .sync-btn-row { display: flex; gap: 8px; }
  .btn-sync-action {
    flex: 1; display: inline-flex; align-items: center; justify-content: center; gap: 6px;
    background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.12);
    color: #e2e8f0; border-radius: var(--radius-md); padding: 9px 10px;
    font-size: 0.8rem; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif;
    transition: all .15s;
  }
  .btn-sync-action:hover { background: rgba(13,148,136,.2); border-color: var(--teal); color: #fff; }
  ...
  .btn-sync-disconnect {
    ...
    color: #f87171; border-radius: var(--radius-md); padding: 8px 14px;
    font-size: 0.75rem; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif;
    width: 100%; transition: all .15s; margin-top: 2px;
  }
  .btn-sync-disconnect:hover { background: rgba(248,113,113,.1); border-color: #f87171; }
  ...
  #sync-topbar.tb-ok    { background: rgba(5,150,105,0.94); color: #fff; }
  #sync-topbar.tb-busy  { background: rgba(217,119,6,0.94); color: #fff; }
  #sync-topbar.tb-error { background: rgba(220,38,38,0.94); color: #fff; }
  ...
  .cl-date-card.open .cl-date-head { border-bottom-color: var(--border); background: #f1f5f9; }
  .cl-date-head:hover { background: #eef2f7; }
  ...
  .cl-date-total {
    ...
    background: #e6f7f4; border-radius: 20px; padding: 3px 10px; white-space: nowrap;
  }
  ...
  .cl-shift-block.mode-final .cl-shift-header { background: #fffbeb; border-bottom-color: #fde68a; }
  ...
  .cl-badge-final { font-size: 0.68rem; font-weight: 700; padding: 2px 7px; border-radius: 10px; background: #fef3c7; color: #92400e; }
  ...
  .cl-open-btn:hover { background: var(--navy); color: #fff; border-color: var(--navy); }
  ...
  .cl-line .cl-lbl { color: var(--muted); font-size: 0.73rem; }
  .cl-line .cl-val { font-weight: 600; font-family: 'JetBrains Mono', monospace; font-size: 0.78rem; }
  .cl-adj-row .cl-lbl { color: #92400e; }
  .cl-adj-row .cl-val { color: #92400e; }
  .cl-total-row {
    display: flex; justify-content: space-between; align-items: center;
  ...
  .cl-show-more:hover { border-color: var(--teal); color: var(--teal-dark); background: #f0fdfa; }
  ...
  .cl-export-row input[type=date], .cl-export-row select {
    font-size: 0.78rem; padding: 5px 8px; border: 1px solid var(--border);
    border-radius: var(--radius-sm); background: #fff; color: var(--slate);
  ...
  .lpb-text {
    font-size: 0.7rem; color: var(--muted); white-space: nowrap; font-weight: 600;
  }
  .lpb-track {
  ...
  .lpb-chip.lpb-pending { background: #fffbeb; color: #b45309; border-color: #fde68a; }
  .lpb-chip.lpb-done {
    background: var(--teal-light); color: var(--teal-dark);
    border-color: var(--teal); font-weight: 700;
  }
  .lpb-chip.lpb-done::after { content: '✓'; font-weight: 800; margin-left: 1px; }
  .lpb-chip.lpb-alert { background: var(--red-pale); color: var(--red); }
  .lpb-chip.lpb-current {
    background: var(--teal); color: #fff; transform: scale(1.1);
  ...
  /* ═══════════════════════════════════════════════════════════
     CARD STATUS EDGE  (left accent stripe — 4-state model)
  ═══════════════════════════════════════════════════════════ */
  .card[data-section] { border-left: 3px solid transparent; transition: border-color .15s; }
  .card[data-section].cs-white    { border-left-color: transparent; }
  .card[data-section].cs-pending  { border-left-color: #f59e0b; }
  .card[data-section].cs-complete { border-left-color: var(--teal); }
  .card[data-section].cs-alert    { border-left-color: var(--red); }
  ...
  .focus-btn.focus-btn-next { background: var(--teal); border-color: var(--teal); color: #fff; }
  .focus-btn.focus-btn-save { background: #fffbeb; border-color: #f59e0b; color: #b45309; flex: 0 0 auto; padding: 0 14px; }
  .focus-btn.focus-btn-save.fb-saved { background: var(--teal-pale); border-color: var(--teal); color: var(--teal-dark); }
  ...
  .viewall-row.va-pending .va-value { color: #b45309; font-weight: 700; }
  .viewall-row.va-complete .va-value { color: var(--teal-dark); font-weight: 700; }
  .viewall-row.va-alert .va-value { color: var(--red); font-weight: 700; }
  ...
  .summary-head p { font-size: 0.72rem; color: #94a3b8; margin: 0; }
  .summary-body { padding: 16px 18px; display: flex; flex-direction: column; gap: 10px; }
  .summary-stat {
    background: #f8fafc; border-radius: var(--radius-md); padding: 12px 14px;
  }
  ...
  .summary-stat.sv-warn  { background: #fffbeb; border: 1px solid #fde68a; }
  .summary-stat.sv-warn .summary-stat-label, .summary-stat.sv-warn .summary-stat-value { color: #b45309; }
  .summary-stat.sv-bad   { background: var(--red-pale); border: 1px solid #fecaca; }
  ...
  .summary-incomplete-note {
    ...
    border: 1px solid #fde68a; border-radius: var(--radius-md); padding: 10px 12px;
    font-size: 0.72rem; color: #92400e; margin: 0 18px 6px;
  }
