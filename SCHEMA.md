...
| `mediqRows` | `{id, billNum, val, pharmBill, deleted}[]` | MEDIQ COD order line items — `billNum` is the MEDIQ order/bill reference (text); `val` is the cash collected from the customer; `pharmBill` is the **matching pharmacy bill amount** (a number, not a reference string). The order's contribution is the difference `val − pharmBill` — the extra collected over the bill, i.e. the delivery/service charge kept. Rows saved before the billNum/pharmBill split carry a `lbl` string instead — `hydrate()` falls back to it. Rows saved while `pharmBill` was a free-text reference parse to `0`, so their extra degrades to the old gross figure rather than breaking. |
| `outPrevMediq` | number | "Previous MEDIQ amount collected" — a **manually entered** carry-in on the MEDIQ card, added on top of this shift's own order extras. Deliberately *not* auto-pulled from the previous shift (unlike `outPrevDep`/`outPrevCash`), because the Final Closing aggregation already sums every shift in the period and an auto-carry would double-count. |
| `outTotalI` | number | Extra MEDIQ collected this shift — `outPrevMediq` + the sum of each non-deleted order's `val − pharmBill`. Folded into **Net Sale (this shift)** (`out-net-sale`), not the Grand Total — which therefore means **cash kept beyond the pharmacy bill value**, not gross cash collected via COD. Not carried forward to the next shift. Records written before this change stored the *gross* COD total here, so `mediqExtraOf()` (state.js) recomputes from `mediqRows` and only falls back to this field when a record has no rows at all. |
| `outFinalMediq` | number | Final Closing aggregation's Extra MEDIQ line. Follows the Book Bills rule: in Final mode it's this shift's `outTotalI` alone; otherwise it's `aggregateSinceLastFinal().totalMediqExtra` (all periods since the last Final, including the last Final's own) plus this shift's. A `+` term in Net Final Sale. |
