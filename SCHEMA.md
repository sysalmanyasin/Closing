...
| `mediqRows` | `{id, billNum, val, pharmBill, deleted}[]` | MEDIQ COD order line items — `billNum` (MEDIQ order/bill ref) and `pharmBill` (matching pharmacy bill #) are reference-only text fields for reconciliation; `val` is the COD amount actually collected for that order. Rows saved before this field existed carry a `lbl` string instead of `billNum`/`pharmBill` — `hydrate()` falls back to it. |
| `outTotalI` | number | Total MEDIQ collected this shift — sum of each order's COD amount collected (`val`). Folded into Grand Total as the `I` term (`A+B+C+D+E+F+G+H+I`). Not carried forward to the next shift. |
