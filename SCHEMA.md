...
| `outTotalI` | number | Extra MEDIQ collected this shift — `outPrevMediq` + the sum of each non-deleted order's `val − pharmBill`. Folded into Grand Total as the `I` term (`A+B+C+D+E+F+G+H+I`), which therefore means **cash kept beyond the pharmacy bill value**, not gross cash collected via COD. Not carried forward to the next shift. Records written before this change stored the *gross* COD total here, so `mediqExtraOf()` (state.js) recomputes from `mediqRows` and only falls back to this field when a record has no rows at all. |
